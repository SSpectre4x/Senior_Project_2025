...
Error::Error* checkUninitializedRegisters(const std::string& line, int lineNum, std::map<std::string, bool>& initializedRegs);
Error::Error* checkRestrictedRegisters(const std::string& line, int lineNum);

// Main function to analyze file for error detection
std::vector<Error::Error> analyzeErrorDetection(std::vector<std::string> lines);
