@ Error Test File
@ 
@ uses r15 for oprations, invalid register to use
@ evenloop calls to undefined subroutine
@ uses OuputNum instead of OutputNumber in [oddloop]
@ numInputPattern has %c instead of %d to read in a number in [.data]
...
@ Take the input from the user and store in intInput
	...
	ldr r15, [r1]                   @read contents of intInput and store
								   @ to print 
...
compare100:
@----------
	cmp r15, #100	               @check for a read error
...
@--------
	...
	cmp r15, #2					   @check for a read error
...
@-------
	...
	ldr r0, =outputNum		       @load string that prints number
...
.balign 4 
numInputPattern: .asciz " %c"         @format to read in an integer
